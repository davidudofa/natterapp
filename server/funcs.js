...
module.exports.findNextMessage = function (inbox, lastHash) {
  ...
  //check if hash was correct
  if (found == null){
    var result = {
      status: 'failed',
      errormsg: 'Incorrect hash key'
    }
  }else{
    // read and decode the message into array
    var result = {
      status: 'success',
      from: decode(inbox.messages[found].from),
      msg: decode(fs.readFileSync(path.join(inbox.dir, inbox.messages[found].hash), 'utf8'))
    }
  }
